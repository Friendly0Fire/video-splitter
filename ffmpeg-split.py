...
def split_by_manifest(filename, manifest, vcodec="copy", acodec="copy", extra="", **kwargs):
    ...
    with open(manifest) as manifest_file:
        ...
        if manifest_type != "json" and filename is None:
            print("Must pass filename with csv manifest")
            raise SystemExit
        ...
        for video_config in config["splits"]:
            split_str = ""
            split_args = []
            try:
                ...
                split_args += ["-ss", str(split_start), "-t", str(split_length), filebase + "." + fileext]
                print("########################################################")
...
def main():
    ...
    parser.add_option(
        "-f", "--file", dest="filename", help="File to split, for example sample.avi", type="string", action="store"
    )
    parser.add_option(
        "-s",
        "--split-size",
    ...
    parser.add_option(
        "-c", "--split-chunks", dest="split_chunks", help="Number of chunks to split to", type="int", action="store"
    )
    parser.add_option(
        "-S",
        "--split-filesize",
    ...
    parser.add_option(
        ...
        help="with --split-filesize, use this factor in time to" " size heuristics [default: %default]",
        type="float",
        action="store",
        default=0.95,
    )
    parser.add_option(
        "--chunk-strategy",
        dest="chunk_strategy",
        help="with --split-filesize, allocate chunks according to" " given strategy (eager or even)",
        type="choice",
        action="store",
        choices=["eager", "even"],
        default="eager",
    ...
    parser.add_option(
        "-v", "--vcodec", dest="vcodec", help="Video codec to use. ", type="string", default="copy", action="store"
    )
    parser.add_option(
        "-a", "--acodec", dest="acodec", help="Audio codec to use. ", type="string", default="copy", action="store"
    )
    parser.add_option(
        "-e",
        "--extra",
    ...
    if not options.filename and not options.manifest:
        bailout()
